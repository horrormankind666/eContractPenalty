...
{  
    ...
    public const int ROW_PER_PAGE = 100;
    ...
    public static string[,,] _actionTrackingStatus = new string[,,]
                                                     {{{"1111", "v1"}, {"1112", "v1"}, {"2111", "a1"}, {"2121", "v1"}, {"2122", "v1"}, {"2211", "e2"}, {"2212", "v2"}},
                                                      {{"1111", "e1"}, {"1112", "v1"}, {"2111", "v1"}, {"2121", "e1"}, {"2122", "v1"}, {"2211", "v2"}, {"2212", "v2"}},                                                      
                                                      {{"1111", "v1"}, {"1112", "v1"}, {"2111", "v1"}, {"2121", "v1"}, {"2122", "v1"}, {"2211", "v2"}, {"2212", "v2"}}};

    public static string[,] _iconTrackingStatus = new string[,]
                                                  {{"1111", "status-send"}, {"1112", "status-cancel"}, {"2111", "status-receiver-disable"}, {"2121", "status-edit"}, {"2122", "status-cancel"}, {"2211", "status-receiver"}, {"2212", "status-cancel"}};
    ...
    public static string[] _iconPaymentStatus = new string[]
                                                {"status-payment-disable", "status-payment-wait", "status-payment-success"};

    public static string[,] _iconEContractStatus = new string[,] 
                                                   {{ "0", "status-e-contract-uncomplete" }, { "1", "status-e-contract-complete" }};

    public static string[,] _dlevel = new string[,] {{"ต่ำกว่าปริญญาตรี", "U"}, {"ปริญญาตรี", "B"}};
    public static string[,] _caseGraduate = new string[,] {{"ก่อนสำเร็จการศึกษา", "ไม่สำเร็จการศึกษา"}, {"หลังสำเร็จการศึกษา", "สำเร็จการศึกษา"}};
    public static string[] _civil = new string[] {"ปฏิบัติงานชดใช้", "ไม่ปฏิบัติงานชดใช้"};
    public static string[] _scholar = new string[] {"ได้รับทุนการศึกษา", "ไม่ได้รับทุนการศึกษา"};
    //ปรับปรุงเมื่อ ๐๓/๐๔/๒๕๖๒
    //---------------------------------------------------------------------------------------------------
    public static string[] _studyLeave = new string[] {"ไม่มีการลาศึกษา / ลาฝึกอบรม ระหว่างการปฏิบัติงานชดใช้ทุน", "มีการลาศึกษา / ลาฝึกอบรม ระหว่างการปฏิบัติงานชดใช้ทุน"};
    //---------------------------------------------------------------------------------------------------
    public static string[,] _trackingStatusORAA = new string[,] {{"รอส่งรายการแจ้ง", "1"}, {"รอรับรายการแจ้ง", "2"}, {"รับรายการแจ้งแล้ว", "3"}, {"ส่งกลับแก้ไข", "4"}, {"รายการแจ้งถูกยกเลิก", "5"}, {"รอส่งรายการแจ้ง, ส่งกลับแก้ไข, รายการแจ้งถูกยกเลิก", "6"}};
    public static string[,] _trackingStatusORLA = new string[,] {{"รอรับรายการแจ้ง", "2"}, {"รับรายการแจ้งแล้ว", "3"}, {"ส่งกลับแก้ไข", "4"}, {"รายการแจ้งถูกยกเลิก", "5"}};
    ...
    public static string[] _resultReply = new string[] {"ผู้ผิดสัญญารับทราบให้ชำระหนี้", "ผู้ผิดสัญญาไม่รับทราบให้ชำระหนี้"};
    public static string[] _paymentStatus = new string[] {"ยังไม่ได้ชำระหนี้", "อยู่ระหว่างการชำระหนี้", "ชำระหนี้เรียบร้อย"};
    public static string[] _paymentFormat = new string[] {"ชำระแบบเต็มจำนวน", "ชำระแบบผ่อนชำระ"};
    public static string[] _payChannel = new string[] {"ชำระด้วยเงินสด", "ชำระด้วยเช็คขีดคร่อม", "นำฝากบัญชีเงินฝาก" };
    public static string[,] _calInterestYesNo = new string[,] {{"คิดดอกเบี้ย", "Y"}, {"ไม่คิดตอกเบี้ย", "N"}};
    ...
    public static double[] CalPenalty(string _scholar, string _caseGraduate, string _civil, string _totalPayScholarship, string _scholarshipYear, string _scholarshipMonth, string _dateStart, string _dateEnd, string _indemnitorYear, string _indemnitorCash, string _calDateCondition, double _addDays)
    {
        ...
        if (!String.IsNullOrEmpty(_dateStart) && !String.IsNullOrEmpty(_dateEnd))
        {
            ...
            switch (_formular)
            {
                case 1: { _totalPenalty = CalPenaltyFormular1(_iCash, _resultCalcDate[3]); break; }
                case 2: { _totalPenalty = CalPenaltyFormular2(_iCash, _resultCalcDate[1], _resultCalcDate[2], _dayLastMonth); break; }
                case 3: { _totalPenalty = CalPenaltyFormular3(_iCash, _allActual, _resultCalcDate[0]); break; }
            }
